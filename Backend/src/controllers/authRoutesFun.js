...
async function register(req, res) {
  try {
    ...
    // 📧 Send welcome mail (safe try-catch)
    try {
      const mail = {
        from: process.env.SENDER_EMAIL,
        to: email,
        subject: "Welcome to ExamPrepHub",
        html: RAGISTER_EMAIL.replace("{{name}}", name)
          .replace("{{email}}", email)
          .replace("{{url}}", process.env.FRONTEND_URL),
      };
...
async function resetPasswordOtp(req, res) {
  try {
    ...
    const mail = {
      ...
      html: PASSWORD_RESET_TEMPLATE.replace("{{otp}}", PassOtp).replace(
        "{{email}}",
        user.email
      ),
    };
